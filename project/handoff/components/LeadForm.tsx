...
/**
 * LeadForm — Greenbelt Electrical estimate request form.
 * Next.js (App Router) client component, Tailwind CSS.
 *
 * Accessibility (WCAG 2.1 AA, per brand guidelines):
 *  - Every field has an explicit, visible <label> (no placeholder-as-label).
 *  - Required fields use aria-required + aria-invalid; errors are linked via
 *    aria-describedby and announced with role="alert".
 *  - Visible focus rings (never outline:none). 44px+ tap targets, 18px text.
 *  - Submit posts JSON to /api/lead (server route forwards to n8n → HCP).
 *
 * Tailwind brand colors expected in tailwind.config (see README):
 *  forest #1E4D2B · green #5CB63B · yellow #F2C829 · charcoal #1A1A1A
 *  conduit #3C3C3C · surface #FAFAF8 · line #D2D2CB
 */
...
export default function LeadForm() {
  ...
  if (status === 'success') {
    return (
      ...
      >
        <div className="mx-auto mb-4 grid h-18 w-18 place-items-center rounded-full border-2 border-green bg-green/10">
  ...
  const fieldBase =
    ...
    'transition-colors focus:border-forest focus:outline-none focus:ring-2 focus:ring-forest/25';
  ...
  return (
    ...
    >
      ...
      <p className="mt-3.5 text-center text-sm text-conduit">
...
/* ---- Field wrapper: label + control + accessible error ---- */
function Field({
  id,
  label,
  required,
  ...
  return (
    <div className="mb-4.5">
      <label htmlFor={id} className="mb-2 block text-base font-bold text-charcoal">
        {label} {required && <span className="text-[#B42318]" aria-hidden="true">*</span>}
      </label>
      {children({
        id,
        name: id,
